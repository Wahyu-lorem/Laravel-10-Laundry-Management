...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const kuponSelect = document.getElementById('kode_kupon');
    const kuponMessage = document.getElementById('kuponMessage');

    function updateTotalPrice() {
        const selectedPaket = paketSelect.options[paketSelect.selectedIndex];
        const pricePerKg = parseFloat(selectedPaket.getAttribute('data-price'));
        const quantity = parseInt(kilogramInput.value);

        if (!isNaN(pricePerKg) && !isNaN(quantity) && quantity > 0) {
            const totalPrice = pricePerKg * quantity;
            hargaInput.value = `Rp ${totalPrice.toFixed(0).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
        } else {
            hargaInput.value = '';
        }
    }

    function updateKuponOptions() {
        const selectedUserId = userSelect.value;
        let kuponFound = false;

        for (let i = 0; i < kuponSelect.options.length; i++) {
            const option = kuponSelect.options[i];
            if (option.getAttribute('data-user-id') === selectedUserId) {
        ...
        kuponSelect.value = '';
        kuponMessage.textContent = kuponFound ? '' : 'Pengguna ini belum mendapatkan kupon.';
    }

    paketSelect.addEventListener('change', updateTotalPrice);
    kilogramInput.addEventListener('input', updateTotalPrice);
    userSelect.addEventListener('change', updateKuponOptions);
